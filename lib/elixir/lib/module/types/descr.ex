...
defmodule Module.Types.Descr do
  ...
  * For gradual tuple types: processes both dynamic and static components separately,
  ...
  defp map_bdd_intersection({_, _, _} = leaf1, {_, _, _} = leaf2) do
    map_leaf_intersection(leaf1, leaf2)
  end

  defp map_bdd_intersection({:closed, _, _} = leaf, bdd) do
    bdd_leaf_intersection(leaf, bdd, &map_leaf_intersection/2)
  end

  defp map_bdd_intersection(bdd, {:closed, _, _} = leaf) do
    bdd_leaf_intersection(leaf, bdd, &map_leaf_intersection/2)
  end

  defp map_bdd_intersection({{:closed, _, _} = leaf, :bdd_top, u, d, _}, bdd) do
    bdd_leaf_intersection(leaf, bdd, &map_leaf_intersection/2)
    ...
    |> case do
      ...
      result ->
        bdd_union(result, bdd_intersection(bdd, bdd_node_new(leaf, :bdd_bot, :bdd_bot, d)))
    end
  end

  defp map_bdd_intersection(bdd, {{:closed, _, _} = leaf, :bdd_top, u, d, _}) do
    bdd_leaf_intersection(leaf, bdd, &map_leaf_intersection/2)
    |> bdd_union(map_bdd_intersection(u, bdd))
    |> case do
      result when d == :bdd_bot ->
  ...
  defp map_bdd_intersection({{:closed, _, _} = leaf, :bdd_bot, u, d, _}, bdd) do
  ...
  defp map_bdd_intersection(bdd, {{:closed, _, _} = leaf, :bdd_bot, u, d, _}) do
    case map_bdd_intersection(u, bdd) do
      result when d == :bdd_bot ->
        result
  ...
  defp tuple_difference({:open, [], _}, {:open, [], _}),
    do: :bdd_bot

  defp tuple_difference({:open, [], _}, bdd2),
    do: bdd_negation(bdd2)
  ...
  # down nodes.
  defp bdd_intersection({_, _, _} = leaf, bdd, leaf_intersection) do
    bdd_leaf_intersection(leaf, bdd, leaf_intersection)
  end

  defp bdd_intersection(bdd, {_, _, _} = leaf, leaf_intersection) do
    bdd_leaf_intersection(leaf, bdd, leaf_intersection)
  end
  ...
  defp bdd_intersection({leaf, :bdd_top, u, d, _}, bdd, leaf_intersection) do
    bdd_leaf_intersection(leaf, bdd, leaf_intersection)
  ...
  defp bdd_intersection(bdd, {leaf, :bdd_top, u, d, _}, leaf_intersection) do
    bdd_leaf_intersection(leaf, bdd, leaf_intersection)
    |> bdd_union(bdd_intersection(u, bdd, leaf_intersection))
    |> case do
      result when d == :bdd_bot ->
  ...
  defp bdd_intersection({leaf, :bdd_bot, u, d, _}, bdd, leaf_intersection) do
  ...
  defp bdd_intersection(bdd, {leaf, :bdd_bot, u, d, _}, leaf_intersection) do
    case bdd_intersection(u, bdd, leaf_intersection) do
      result when d == :bdd_bot ->
        result
  ...
  defp bdd_leaf_intersection(leaf, bdd, intersection) do
    leaf = bdd_normalize(leaf)
    bdd = bdd_normalize(bdd)
    ...
    case bdd do
      ...
      {lit, c, u, d, _} ->
        if bdd_equal?(lit, leaf) do
          case bdd_union(c, u) do
            :bdd_bot -> :bdd_bot
            cu -> bdd_split(lit, cu, :bdd_bot, :bdd_bot)
          end
        else
          rest =
            bdd_union(
